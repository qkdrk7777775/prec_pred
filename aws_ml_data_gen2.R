...
load("/home/ducj/R/git/kma/aws.rda")
...
#station=244부터 
for(i in station2){
  if(!c(paste0('aws_',i,'.csv'))%in%list.files('/home/ducj/R/git/kma/aws_temp/')){
    write.csv(NULL,paste0('/home/ducj/R/git/kma/aws_temp/aws_',i,'.csv'))
    list1=list()
    message(i, '/',max(station2))
    list1[[i]]=df2[df2$station==i,]
    #결측 없는 데이터 생성
    ...
    write.csv(list1[[i]],paste0('/home/ducj/R/git/kma/aws_temp/aws_',i,'.csv'))
  }  
}
