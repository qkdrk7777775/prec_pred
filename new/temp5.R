...
train$station=NA
train=train[year(train$times)==2019,]
...
imputeTSRFsp=function(train, grid.dist,station_unique_name=c('lon','lat','alt','station')){
  ...
  #save(raw_df2,full_df_temp2,file='/home/ducj/jupyter/prec_pred/new/temp2.rda')
  load(file='/home/ducj/jupyter/prec_pred/new/temp2.rda')
  ...
  if(any(is.na(full_df_temp2))){
    ...
    for(target_temp in names(na_count[na_count!=0])){
      ...
      save(full_df_temp2,file=paste0('/home/ducj/jupyter/prec_pred/new/',cnt,'_',length(na_count),target_temp,'.rda'))
    }
  }
  ...
  load(file='/home/ducj/jupyter/prec_pred/new/9_14w_temp.rda')
  ...
  summary(full_df2)
    ...
    for(target in col_names){
      for(date_temp in date_list2){
        ...
        write_csv(data.frame(df_pixel0@coords,predict(m.zinc2,grid.dist@data)$prediction),file=paste0('/home/ducj/jupyter/prec_pred/new/temp/',target,date_temp,'.csv'))
      }
